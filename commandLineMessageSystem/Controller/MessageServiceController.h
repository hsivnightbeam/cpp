...
public:
    ...
    const hashSetUser::iterator                         find   (const std::string &userName)         { return users.find(userName); }
    ...
    //Members related to data member messages
          hasMaphUserReciMsg&  getUserReceiveMsgsIndex() { return messages.getUserReceiveMsgsIndex(); }
